...
axios
  .get(
    "https://project-1-api.herokuapp.com/comments?api_key=3199f46c-ce0e-4840-80c4-0781158980ed"
  )
  .then(response => {
    let commentsFromApi = response.data;
    console.log(commentsFromApi);
    console.log(commentsFromApi.reverse());
    for (let x = 0; x < commentsFromApi.length; x++) {
      let theArray = [x];
      generateDefaultComment(commentsFromApi[x]);
    }
  })
...
function generateDefaultComment(comment) {
  let comments = document.querySelector(".defaultcomment");
  ...
  let commenterDate = document.createElement("h3");
  commenterDate.classList.add("defaultcomment__date");
  let g = comment.timestamp;
  let d = new Date(g);
  let commentDate = d.getUTCDate();
  let commentMonth = d.getUTCMonth();
  let commentYear = d.getUTCFullYear();
  commenterDate.innerHTML = `${commentDate}/ ${commentMonth}/ ${commentYear}`;

  let commenterComment = document.createElement("p");
  commenterComment.classList.add("defaultcomment__comment");
  commenterComment.innerText = comment.comment;
...
form.addEventListener("submit", event => {
  ...
  axios
    ...
    .then(function(response) {
      ...
      //reload the comments calling the get comment function
      // wrap the get comment inside a function
    })
    .catch(function(error) {
      console.log("stupid");
    });
